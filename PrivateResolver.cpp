...
#define RandomInt(nMin, nMax) (rand() % (nMax - nMin + 1) + nMin);
			...
			CResolver::newsimtime = pEntity->GetSimulationTime();
				...
				lbyupdated = true;
			{
				if (Menu::Window.RageBotTab.AntiAimCorrection.GetState())
				{
					...
					if (LatestLowerBodyYawUpdateTime[pEntity->GetIndex()] < pEntity->GetSimulationTime() || bLowerBodyIsUpdated)
					{
						...
						{
							switch (pEntity->resolver_data.shots % 4)//logging hits for everyhitgroup//not anymore
							{
							case 0:
						...
						else
						{
							...
							else if (MeetsLBYReq && !IsMoving && pEntity->GetHealth() > CResolver::storedhealth[pEntity->GetIndex()])
							{
								...
								pEntity->GetEyeAngles()->y = v20;
								{
									...
									{
										switch (pEntity->resolver_data.missedshots % 5)
...
void CResolver::StoreFGE(IClientEntity* pEntity)
{
	...
	CResolver::storedsimtimeFGE = pEntity->GetSimulationTime();
}
...
void CResolver::StoreThings(IClientEntity* pEntity)
{
	...
	CResolver::storedsimtime = pEntity->GetSimulationTime();
...
void CResolver::LowerBodyYawFix(IClientEntity* pEntity)
{
	...
	bool NextLBYUpdate();
	{
		...
		float flServerTime = (Interfaces::Globals->tickcount);
		...
		float servertime = (float)(tick * Interfaces::Globals->interval_per_tick);
